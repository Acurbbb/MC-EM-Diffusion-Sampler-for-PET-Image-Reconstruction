...
class GaussianDiffusion():
    ...
    @torch.no_grad()
    def p_sample(self, model, x, t: int, model_kwargs=None, clip_denoised=True):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((b,), t, device = x.device, dtype = torch.long)
        model_mean, variance, model_log_variance, x_start = self.p_mean_variance(
           model=model, x=x, t=batched_times, model_kwargs=model_kwargs, clip_denoised=clip_denoised
        )
    ...
    @torch.no_grad()
    def p_sample_loop(self, model, shape, save_steps=0, model_kwargs=None, clip_denoised=True):
        batch = shape[0]
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            img, x_start = self.p_sample(model, img, t, model_kwargs=model_kwargs, clip_denoised=clip_denoised)
    ...
    @torch.no_grad()
    def ddim_sample(self, model, shape, save_steps=0, model_kwargs=None, clip_denoised=True):
        batch, total_timesteps, sampling_timesteps = shape[0], self.num_timesteps, self.sampling_timesteps
        device = next(model.parameters()).device
        ...
        for i, (time, time_next) in enumerate(tqdm(time_pairs, desc = 'sampling loop time step')):
            ...
            pred_noise, x_start, *_ = self.model_predictions(model, img, time_cond, model_kwargs=model_kwargs, clip_x_start=clip_denoised)
    ...
    @torch.no_grad()
    def sample(self, model, shape, save_steps=False, model_kwargs=None):
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn(model, shape, save_steps=save_steps, model_kwargs=model_kwargs)
    ...
    def p_losses(self, model, x_start, t, model_kwargs=None, noise = None):
        ...
        model_out = model(x, t, **(model_kwargs or {}))
...
class ConditionalDiffusion(GaussianDiffusion):
    ...
    def sample_mcem_ddim(
        ...
        imgs, x0s = [], []
        for i, (time, time_next) in enumerate(tqdm(time_pairs, desc = 'sampling loop time step')):
            img.detach_().requires_grad_(True)
            time_tensor = torch.full((1,), time, device=device, dtype=torch.long)
            # 1: get unconditional x_0_hat
            ...
            # 2: Get EM update vector
            x_start_scaled = unnormalize_to_zero_to_one(x_start.detach().cpu().numpy().squeeze())
            x_start_scaled_0 = np.copy(x_start_scaled)
            
            for _ in range(cond_schedule[-1-i]):
                x_start_scaled = em_update_fn(x_start_scaled)

            increm = x_start_scaled - x_start_scaled_0
            increm = increm * 2 # 2 for [-1,1] normalization
            increm = torch.from_numpy(increm.astype(np.float32)).to(device)[None, None, ...]

            # 3: Manifold constraint
            increm_mc = torch.autograd.grad(
                outputs=x_start, 
                inputs=img, 
                grad_outputs=increm,
            )[0]
            ...
            alpha = self.alphas_cumprod[time]
            increm_mc *= torch.sqrt(alpha) # scaling factor \sqrt{\bar{alpha}} in the Jacobian
            increm_mc = increm_mc.clamp(min=-5., max=5.)

            if time_next < 0:
                img = x_start + increm_mc
                if save_steps > 0:
            ...
            img = (x_start + increm_mc) * alpha_next.sqrt() + \
                    c * pred_noise + \
                    sigma * noise
        ...
        if save_steps == 0:
            ret = img.detach().cpu().numpy().squeeze()
        else:
    ...
    def sample_pet_dds(
        ...
        for i, (time, time_next) in enumerate(tqdm(time_pairs, desc = 'sampling loop time step')):
            ...
            for _ in range(cond_schedule[-1-i]):
                x_start_scaled = em_update_fn(x_start_scaled, x0=x_start_scaled_0)
    ...
    def sample_dps(
        ...
        for i, (time, time_next) in enumerate(tqdm(time_pairs, desc = 'sampling loop time step')):
            ...
            grad = em_update_fn(x_start_scaled)
            ...
            model_mean, _, posterior_log_variance = self.q_posterior(
                x_start=x_start, x_t=img, t=time_tensor
            )
