...
def get_cli_conf():
    ...
    parser.add_argument("--output_dir", type=str, 
                        default="./results/2d_tmp", help="output dir")
    parser.add_argument("--base", type=str, 
                        default="./configs/train_2d.yaml", help="path to base configs. Parameters can be overwritten or added with command-line options of the form `--key value`.")
    ...
    for epoch in epoch_bar:
        ...
        for big_bat_x in train_bar:
            ...
            if cfg.save_step>0 and global_step % cfg.save_step==0 and rank==0:
                _, _, *image_shape = big_bat_x.shape
                n_samples = 64
                ema_model.eval()
                with torch.no_grad():
                    samples = diffusion.sample(model=ema_model, shape=(n_samples, 1, *image_shape), save_steps=0).squeeze().cpu()
                    samples = ds_train.unnormalize(samples)
                ema_model.train()
                
                plot_image_grid(samples, n=int(n_samples**0.5), vmin=0., vmax=1., 
